...
def build_styles():
    ...
    return {
        'title': ParagraphStyle(
            ...
            spaceAfter=8, spaceBefore=4, alignment=TA_LEFT,
            leading=32,
        ),
        'subtitle': ParagraphStyle(
            'Subtitle', fontName='Helvetica', fontSize=11,
            textColor=colors.HexColor('#888888'),
            spaceAfter=16, spaceBefore=2, alignment=TA_LEFT,
            leading=16,
        ),
        ...
        'stat_label': ParagraphStyle(
            'StatLabel', fontName='Helvetica', fontSize=9,
            textColor=colors.HexColor('#888888'),
            alignment=TA_CENTER,
        ),
        'stat_val': ParagraphStyle(
            'StatVal', fontName='Helvetica-Bold', fontSize=20,
            textColor=colors.HexColor('#f97316'),
            alignment=TA_CENTER,
        ),
        'tag': ParagraphStyle(
...
def export_report(
    ...
    story.append(Spacer(1, 4))
    story.append(HRFlowable(width=W, thickness=1, color=colors.HexColor('#f97316'), spaceAfter=16))

    # ── Stats ─────────────────────────────────────────────────────────────────
    total_mins = db.query(func.sum(models.Log.time_spent)).filter(
        models.Log.user_id == current_user.id
    ).scalar() or 0
    total_logs    = db.query(models.Log).filter(models.Log.user_id == current_user.id).count()
    topics_total  = db.query(models.Topic).filter(models.Topic.owner_id == current_user.id).count()
    mastered      = db.query(models.Topic).filter(models.Topic.owner_id == current_user.id, models.Topic.status == models.StatusEnum.mastered).count()
    resources_cnt = db.query(models.Resource).filter(models.Resource.owner_id == current_user.id).count()
    goals_done    = db.query(models.Goal).filter(models.Goal.owner_id == current_user.id, models.Goal.is_completed == True).count()
    ...
    stat_table = Table(stat_data, colWidths=[W/6]*6)
    ...
    ).order_by(models.Topic.status).all()

    if topics:
        story.append(Paragraph("Topics", styles['section']))
        story.append(HRFlowable(width=W, thickness=0.5, color=colors.HexColor('#eeeeee'), spaceAfter=8))

        STATUS_LABEL = {'to_learn': 'To Learn', 'learning': 'Learning', 'mastered': 'Mastered'}
    ...
    logs = db.query(models.Log).filter(
        models.Log.user_id == current_user.id
    ).order_by(models.Log.date.desc()).limit(15).all()
    ...
    if goals:
        ...
        g_table = Table(g_data, colWidths=[W*0.45, W*0.18, W*0.18, W*0.19], repeatRows=1)
